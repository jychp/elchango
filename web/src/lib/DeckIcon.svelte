<script lang="ts">
  ...
  type PhosphorIconName = Exclude<DeckIconName, 'cursor' | 'claude'>
  ...
  let PhosphorIcon = $derived(
    name === 'cursor' || name === 'claude' ? null : phosphorIcons[name],
  )
</script>
...
{:else if PhosphorIcon}
  <PhosphorIcon size="100%" weight="bold" aria-hidden="true" />
{/if}
