...
@Suite("Codex plugin inspector")
struct CodexPluginInspectorTests {
    ...
    @Test("missing Codex is reported conservatively")
    func codexNotDetected() {
        let inspector = CodexPluginInspector(
            expectedVersion: "1.1.0",
            pluginsRootURL: makeRootURL()
        )
        #expect(inspector.classify(codexInstalled: false) == .codexNotDetected)
    }
    ...
    func pluginMissing() throws {
        ...
        let state = inspector.classify(codexInstalled: true)
    ...
    func managed() throws {
        ...
        let local =
            ...
            .appendingPathComponent("elchango", isDirectory: true)
        try writeManifest(version: "1.1.0", toPluginDirectory: local)
        defer { try? FileManager.default.removeItem(at: root) }
        ...
        let state = inspector.classify(codexInstalled: true)
        #expect(state == .managed(version: "1.1.0"))
        #expect(!state.needsInstall)
    ...
    func matching() throws {
        ...
        try writeManifest(version: "1.1.0", toPluginDirectory: directory)
        defer { try? FileManager.default.removeItem(at: root) }
        ...
        #expect(
            inspector.classify(codexInstalled: true) == .matching(version: "1.1.0")
        )
    }
    ...
    func mismatched() throws {
        ...
        let state = inspector.classify(codexInstalled: true)
        #expect(state == .mismatched(installed: "1.0.0", expected: "1.1.0"))
        #expect(state.needsUpdate)
    }
    ...
    func malformed() throws {
        ...
        #expect(inspector.classify(codexInstalled: true) == .malformed)
    }
    ...
    func foreignIdentityIsNotOurs() throws {
        ...
        #expect(inspector.classify(codexInstalled: true) == .pluginMissing)
    }
